import React from "react";

const GetEmailed = () => {
  return (
    <section className="px-3 py-10">
      <h2 className="text-3xl md:text-5xl uppercase font-semibold max-w-5xl tracking-wide mb-7">
        NEED A PHOTOGRAPHER? SOMEONE WITH EXPERIENCE TO COLLABORATE WITH? WRITE
        ME!
      </h2>
      <a href="mailto:rodelenz@gmail.com" className="text-2xl md:text-5xl font-bold inline-block showline relative">rodelenz@gmail.com</a>
    </section>
  );
};

export default GetEmailed;
